...
async function handleDeleteMaterial(material: Material) {
  try {
    await ElMessageBox.confirm(
      `确定删除资料「${material.title}」？这只会删除本课程副本中的资料，不影响公共课程源内容。`,
      '删除确认',
      { type: 'warning' },
    )
...
<template>
  <div class="course-detail-manage-page" v-loading="loading">
      ...
      <div v-else class="stage-list">
        <div v-for="stage in sortedStages" :key="stage.id" class="stage-row">
          ...
          <el-input-number
            ...
            controls-position="right"
            style="width: 100px"
            @change="handleStageOrderChange(stage)"
          />
    ...
    <div class="section-card materials-section">
      ...
      <el-empty
        v-if="!sortedStages.length && !course?.uncategorized_materials.length"
        description="暂无资料"
      />
    </div>
    ...
    <el-dialog v-model="materialDialogVisible" :title="isEditMaterial ? '编辑资料' : '上传资料'" width="520px">
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="uploading" @click="handleSaveMaterial">
          {{ isEditMaterial ? '保存' : '上传' }}
        </el-button>
      </template>
    </el-dialog>
