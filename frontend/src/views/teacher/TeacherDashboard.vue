...
// ── 快捷操作 ──
// 「管理课程」与侧栏、列表操作一致：优先进入最近管理的课程详情
function resolveManageCoursePath() {
  try {
    const id = Number(sessionStorage.getItem('teacher_last_managed_course_id') || 0)
    if (Number.isFinite(id) && id > 0) return `/teacher/courses/${id}`
  } catch {
    // ignore
  }
