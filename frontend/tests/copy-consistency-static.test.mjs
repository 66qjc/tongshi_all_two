...
const adminQuestionBank = read('src/views/admin/AdminQuestionBank.vue')
...
test('教师端危险操作和空状态应说明真实后果和下一步', () => {
  ...
  assert.match(teacherCourseDetail, /description="暂无资料"/, '课程详情应有暂无资料空状态')
...
test('导入说明应有中文解释且避免纯内部枚举口吻', () => {
  ...
  assert.match(adminQuestionBank, /choice（选择题）/, '管理员共享题库导入说明应解释 choice')
  assert.match(adminQuestionBank, /multi_choice（多选题）/, '管理员共享题库导入说明应解释 multi_choice')
  assert.match(adminQuestionBank, /fill（填空题）/, '管理员共享题库导入说明应解释 fill')
})
...
test('管理端文案承诺应和实际行为一致', () => {
  ...
  assert.doesNotMatch(adminPublicCourses, /showQuestionDialog|openCreateQuestion|openImportQuestions/, '公共课程页不应再内嵌题库维护弹窗')
...
test('学生端应统一公开教程、学习资料和作业术语', () => {
  ...
  assert.doesNotMatch(learn, /抽出一门课程|翻看简介|右侧会抽出展开|选一本书/, '学页说明不应依赖书架隐喻')
  assert.match(moduleShowcase, /进入公开学习馆查看课程与学习资料/, '首页学习入口状态可保留公开学习馆口径')
  ...
  assert.match(courseDetail, /学习资料/, '课程详情应使用学习资料术语')
  assert.match(courseDetail, /资料目录/, '课程详情应保留资料目录')
  assert.match(courseDetail, /MaterialInlineReader/, '课程详情应以资料直读为主路径')
  assert.doesNotMatch(courseDetail, /本课资源|课程资料库|本课学习资料|查看全部学习资料/, '学生端不应混用旧资源口径或课时页签文案')
