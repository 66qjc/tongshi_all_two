...
assert.match(courseDetail, /MaterialInlineReader/, '学生课程详情应使用直读式资料阅读器。')
	...
	assert.match(courseDetail, /function materialSummary\(material: Material\)/, '课程详情应有摘要展示函数。')
	assert.match(courseDetail, /material\.type !== 'pdf'/, '右侧导读应仅对 PDF 强调抽取摘要。')
	assert.match(courseDetail, /PDF 摘要生成中/, 'PDF 摘要生成中应有中文状态提示。')
	assert.match(inlineReader, /props\.material\.type === 'pdf'/, '直读器应仅对 PDF 展示抽取摘要。')
	assert.match(inlineReader, /PDF 摘要生成中/, '直读器应对 PDF 摘要生成中显示中文提示。')
